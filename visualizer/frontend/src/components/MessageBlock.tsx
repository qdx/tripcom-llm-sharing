...
function blockHeight(tokenCount: number): number {
  // Proportional height: min 60px, max 400px, scales with tokens
  const base = 60;
  const scale = Math.log2(Math.max(tokenCount, 1) + 1) * 25;
  return Math.min(Math.max(base, base + scale), 400);
}
...
export function MessageBlock({ message, mode, animationDelay }: MessageBlockProps) {
  ...
  const isExpandable = mode === 'harness' && message.role === 'system';
      ...
      >
        ...
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            {message.section && (
              <span className="text-xs text-gray-500 italic">{message.section}</span>
            )}
          </div>
